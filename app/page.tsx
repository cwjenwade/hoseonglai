...
function findSelectedItem<T>(
  ...
  for (const selectedId of ids.map((id) => String(id || "").trim()).filter(Boolean)) {
    const item = items.find((candidate) => getId(candidate) === selectedId);
    if (item) return item;
  }
...
export default async function Home() {
  const [homeContentValue, videos, groups, lectures, researchProjectsContent, brandContentValue] = await Promise.all([
  ...
  const researchControl = getHomeSectionControl(homeContent, "research");
  const gatheringsControl = getHomeSectionControl(homeContent, "gatherings");
  const programsCollaborationsControl = getHomeSectionControl(homeContent, "programsCollaborations");
  const aboutControl = getHomeSectionControl(homeContent, "about");
  const researchCta = getHomeSectionCta(researchControl);
  const gatheringsCta = getHomeSectionCta(gatheringsControl);
  const programsCta = getHomeSectionCta(programsCollaborationsControl, "programs");
  const collaborationsCta = getHomeSectionCta(programsCollaborationsControl, "collaborations");
  const aboutCta = getHomeSectionCta(aboutControl);
  // Empty or unmatched selectedIds intentionally falls back to the previous homepage selection behavior.
  const featuredResearch =
    findSelectedItem(videos, researchControl.selectedIds, (video) => video.tag) ||
    videos.find((video) => video.tag === "alexithymia") ||
    videos[0] ||
    HEARTFELT_VIDEOS[0];
  const featuredGathering =
    findSelectedItem(groups.filter(isGroupVisible), gatheringsControl.selectedIds, (group) => group.slug) ||
    groups.filter(isGroupVisible)[0] ||
    GROUPS.find(isGroupVisible) ||
    GROUPS[0];
  const publishedProjects = normalizeResearchProjects(researchProjectsContent, RESEARCH_PROJECTS);
  const featuredCollaboration =
    findSelectedItem(
      publishedProjects,
      programsCollaborationsControl.selectedIds,
      (project) => project.id,
      "project",
    ) ||
    publishedProjects[0] ||
    normalizeResearchProjects(RESEARCH_PROJECTS)[0];
  const featuredProgram =
    ...
    findSelectedItem(lectures, programsCollaborationsControl.selectedIds, (lecture) => lecture.slug, "lecture") ||
    pickUpcomingLecture(lectures) ||
    LECTURES[0];
  ...
  function renderHomeSection(sectionKey: HomeSectionKey) {
    switch (sectionKey) {
      ...
      case "research":
        return (
          <section key={sectionKey} className="px-6 pb-8 pt-2 md:px-10 md:pb-12">
      ...
      case "gatherings":
        return (
          <section key={sectionKey} className="px-6 py-10 md:px-10 md:py-14">
      ...
      case "programsCollaborations":
        return (
          <section key={sectionKey} className="px-6 py-12 md:px-10 md:py-16">
            <div className="mx-auto max-w-[1520px]">
              <div className="grid gap-6 lg:grid-cols-2">
                ...
                <article className="rounded-[2rem] border border-black/8 bg-[#fbf8f2] p-7 md:p-9">
                  ...
                  <Link
                    href={collaborationsCta.href}
                    className="mt-8 inline-flex items-center gap-3 text-[0.8rem] uppercase tracking-[0.28em] text-black/72 transition hover:opacity-62"
                    style={{ fontFamily: "var(--font-geist-sans)" }}
                  >
                    {collaborationsCta.label}
                    <span aria-hidden="true">↗</span>
                  </Link>
                </article>
              </div>
      ...
      case "about":
        return (
          <section key={sectionKey} className="px-6 pb-20 pt-4 md:px-10 md:pb-24">
